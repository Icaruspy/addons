...
class supplier_reconcile(Model):
    '''
    A supplier reconcile is the document that associates already made payments with debts of the supplier
    '''
    ...
    def _get_balance(self, cr, uid, ids, name, args, context=None):
        if not ids: return {}
        res = {}
        for srec in self.browse (cr, uid, ids, context=context):
            credits = []
            for credit in srec.credits:
                credits.append({'amount': credit.amount})
            debts = []
            for debt in srec.debts:
                debts.append({'amount': debt.amount})

            res[srec.id] = self._compute_balance (cr, uid, credits, debts)
        return res
    ...
    _name = 'account.supplier_reconcile'
    _description = 'Reconciliation of unreciled supplier documents'
    ...
    _columns = {
        ...
        'account': fields.many2one('account.account', 'Account', 
                            ...
                            readonly=True, 
                            states={'draft':[('readonly',False)]}),
        'credits': fields.one2many('account.supplier_reconcile_credit',
                            'srec',
                            'Non reconciled credits', 
                            readonly=True, 
                            states={'draft':[('readonly',False)]}),
        'debts': fields.one2many('account.supplier_reconcile_debt',
                            'srec',
                            'Applied debts', 
                            readonly=True, 
                            states={'draft':[('readonly',False)]}),
    ...
    def name_get(self, cr, uid, ids, context=None):
        if not ids:
            return []
        if context is None: context = {}
        supplier_reconciles = self.browse (cr, uid, ids, context=context)
        return [(srec.id, "%s [%s]" % (srec.name, srec.company_id.name)) for srec in supplier_reconciles]
    ...
    def onchange_company(self, cr, uid, ids, 
        ...
        if company_id:
            ...
            domain['journal'] = [('type','=','purchase'),('company_id','=',company_id)]
    ...
    def action_cancel(self, cr, uid, ids, context=None):
        ...
        for srec in self.browse(cr, uid, ids, context=context):
            val = {
                'state':'canceled',
                'cancelation_move': srec.move and \
                    move_obj.revert(cr, uid, [srec.move.id], 
                                    {'name': srec.name,
                                     'ref': _('CANCELATION'),
                                     'date': fields.date.context_today(self, cr, uid, context=context)}, 
                                     context=context)[srec.move.id] or False, 
            }
            srec.write(val)
    ...
    def action_get_credit_and_debts(self, cr, uid, ids, context=None):
        ...
        for srec in self.browse(cr, uid, ids, context=context):
            if (not srec.period) or (not srec.company) or (not srec.currency) or (not srec.partner):
                continue

            if srec.state != 'draft':
                continue
                
            company = srec.company
            ...
            domain = self.get_lookup_criteria(cr, uid, [srec.id], 
                                              srec.partner.id, 
                                              srec.company.id, 
                                              period.fiscalyear_id.id, 
                                              context=context)

            for credit in srec.debits:
                credit.unlink()
            for debt in srec.debts:
                debt.unlink()
            ...
            srec.write({'credits': [(0, 0, c) for c in credits], 
    ...
    def action_reasign_credit(self, cr, uid, ids, context=None):
        ...
        for srec in self.browse (cr, uid, ids, context=context):
            ...
            total_credit = 0.0
            for credit in srec.cash_entries:
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        return super(supplier_reconcile, self).unlink(cr, uid, ids, context=context)
    ...
    def action_post(self, cr, uid, ids, context=None):
        ...
        for srec in self.browse(cr, uid, ids, context=context):
            ...
            context_multi_currency.update({'date': srec.date})

            if srec.name == '/' or not srec.name:
                name = seq_obj.get(cr, uid, 'supplier_voucher') or '/'

            srec.write ({'name': name})
            srec.refresh()
            ...
            debt_to_apply = list(srec.debits)
            already_applied = 0.0

            move_id = None
            reconcile_lines = []
            ncontext=context.copy()
            ncontext['default_period_id'] = srec.period.id
            ncontext['default_journal_id'] = srec.journal.id

            for credit in srec.uac_ids:
                ...
                if not move_id:
                    vals = {
                        'name': name,
                        'journal_id': srec.journal.id,
                        'narration': srec.narration,
                        'date': srec.date,
                        'ref': srec.notes,
                        'period_id': srec.period.id,
                    }
            ...
            srec.write({'state': 'posted', 'move': move_id})
    ...
    def copy(self, cr, uid, id, default={}, context=None):
        ...
        return super(supplier_reconcile, self).copy(cr, uid, id, default, context)

class supplier_reconcile_debt(Model):
    _name = 'account.supplier_reconcile_debt'
    _description = 'supplier reconcile Debts'
    _order = "due_date,name"

    _columns = {
        'srec': fields.many2one('account.supplier_reconcile', 'supplier Reconcile', ondelete='cascade'),
...
class supplier_reconcile_credit(Model):
    _name = 'account.supplier_reconcile_credit'
    _description = 'supplier reconcile credit'
    _order = "due_date,name"
